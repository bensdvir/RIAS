...
def kFoldNueralNetwork():
    ...
    for ind in range(1,6):
        ...
        actorsTrainVecs2 = np.vectorize(lambda x: True if x == 'Doctor' else False)(np.array(actorsTrain))
        i = 0
        tmpPred = []
        tmpPred2 = []
        ...
        modelDoctor.compile(optimizer='rmsprop',
                      loss='categorical_crossentropy',
                      metrics=['accuracy'])
        ...
        modelPaitent.compile(optimizer='rmsprop',
                      loss='categorical_crossentropy',
                      metrics=['accuracy'])
        ...
        afterFitLabelsTest = tokenizer2.texts_to_matrix([i[1] for i in test_data])
        ###############################################################################################################
        actorsTestVecs2 = np.vectorize(lambda x: True if x == 'Doctor' else False)(np.array(actorsTest))
        i = 0
        tmpTestPred = []
        tmpTestPred2 = []
        ...
        score1 = modelDoctor.evaluate(doctorTestSentences,afterFitLabelsTestDoctor)
        score2 = modelPaitent.evaluate(paitentTestSentences,afterFitLabelsTestPaitent)
        sumScores+= score1[1] + score2[1]
        
        pred1 = modelDoctor.predict(doctorTestSentences)
        pred2 = modelPaitent.predict(paitentTestSentences)
        print (type(pred1))
        pred1List = pred1.tolist()
        pred2List = pred2.tolist()
        t = actorsTestVecs2.tolist()
        tmpL = []
        i1=0
        i2=0        
        for p in t:
            if (p):
                tmpL.append(pred1List[i1])
                i1+=1
            else:
                tmpL.append(pred2List[i2])
                i2+=1
        predicts = np.asarray(tmpL)
        print (predicts.shape)

                
    
        dic = tokenizer2.word_index
        model_predicts = predicts
        tokenizer_dic = dic
    ...
    print ("average accuracy: " +str(sumScores/10))
    workbook4.close()
    return predicts, dic
...
def main():
    model_predicts, tokenizer_dic = kFoldNueralNetwork()
    #calculateComposites()
