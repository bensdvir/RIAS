...
resourcesPath = "englishResources\\"
outputsPath = "englishOutputs\\"
...
dict2 = {'personal':1,'laughs':2,'approve':3,'comp':4,'agree':5,'bc':6,'emp':7
        ,'concern':8,'r/o':9,'legit':10}  

doctorDict ={'orient':1,'gives-thera':2,'gives-med':3,'[?]med':4, 'agree':5, 'c-med/thera':6}
patientDict ={'gives-l/s':1, 'concern':2,'agree':3,'gives-med':4}


categoriesDoctorPrecision = {'orient':0,'gives-thera':0,'gives-med':0,'[?]med':0, 'agree':0, 'c-med/thera':0}
categoriesPatientPresicion = {'gives-l/s':0, 'concern':0,'agree':0,'gives-med':0}

categoriesDoctorRecall = {'orient':0,'gives-thera':0,'gives-med':0,'[?]med':0, 'agree':0, 'c-med/thera':0}
categoriesPatientRecall = {'gives-l/s':0, 'concern':0,'agree':0,'gives-med':0}

categoriesDoctorTrue = {'orient':0,'gives-thera':0,'gives-med':0,'[?]med':0, 'agree':0, 'c-med/thera':0}
categoriesPatientTrue = {'gives-l/s':0, 'concern':0,'agree':0,'gives-med':0}

categoriesDoctorRealCategory = {'orient':0,'gives-thera':0,'gives-med':0,'[?]med':0, 'agree':0, 'c-med/thera':0}
categoriesPatientRealCategory = {'gives-l/s':0, 'concern':0,'agree':0,'gives-med':0}

categoriesDoctorPredicted = {'orient':0,'gives-thera':0,'gives-med':0,'[?]med':0, 'agree':0, 'c-med/thera':0}
categoriesPatientPredicted = {'gives-l/s':0, 'concern':0,'agree':0,'gives-med':0}


train_data = []
workbook =xlrd.open_workbook(outputsPath+'\data.xlsx')
...
actorsTrainVecs = np.vectorize(lambda x: 0 if x == 'Doctor' else 1)(np.array(actorsTrain)).reshape((-1,1))
actorsTestVecs = np.vectorize(lambda x: 0 if x == 'Doctor' else 1)(np.array(actorsTest)).reshape((-1,1))
...
def kFoldNueralNetwork(f,alpha,workSheetTemp,workSheetCategories):
    ...
    #row = 1
    workbook4 =xlsxwriter.Workbook(outputsPath + 'composites_common.xlsx')
    ...
    for ind in range(1,6):
        row = 1
        path = outputsPath+ 'english82Comperation_' + str(ind)+'_Common_alpha='+ str(alpha)+'.xlsx'
        workbook9 =xlsxwriter.Workbook(path)
    #worksheet2 = workbook2.sheet_by_name('Sheet1')
        worksheet9 = workbook9.add_worksheet()

        worksheet9.write(0, 0, "Sentence")
        worksheet9.write(0, 1, "Real_tag")
        worksheet9.write(0, 2,  "Prediction")
        worksheet9.write(0, 3,  "Actor")
        ...
        tokenizer1 = keras.preprocessing.text.Tokenizer(filters='!"#$%&*+,.:;<=>@\^_`{|}~\t\n')
        actorsTrainVecs = np.vectorize(lambda x: 1 if x == 'Doctor' else 0)(np.array(actorsTrain)).reshape((-1,1))
        actorsTestVecs = np.vectorize(lambda x: 1 if x == 'Doctor' else 0)(np.array(actorsTest)).reshape((-1,1))
        tokenizer1.fit_on_texts([i[0] for i in train_data])
        tokenizer1.fit_on_texts([i[0] for i in test_data])
        afterFitText = tokenizer1.texts_to_matrix([i[0] for i in train_data], mode="tfidf")      
        ...
        for i in range (0,len(afterFitTextTest)):
            ...
            if (pred!='Unknown'):
                ...
                unknownPred+=1
            worksheet9.write(row, 0, test_data[i][0])
            worksheet9.write(row, 1, test_data[i][1])
            worksheet9.write(row, 2,  pred)
            worksheet9.write(row, 3,  actorsTest[i])
            row+=1
        ...
        precision_ratio+=truePredicts/(len(afterFitTextTest)-unknownPred)
        print ("temp precision:" + str(truePredicts/(len(afterFitTextTest)-unknownPred)))
        ...
        #f.flush()
        f.write(str(alpha)+"(tape"+str(ind)+")           "+str(truePredicts/(len(afterFitTextTest)-unknownPred))+"               "+str(truePredicts/len(afterFitTextTest))+"                     "+str(unknownPred/len(afterFitTextTest)))
        f.write("\n")
        f.flush()
        K.clear_session()
        
    print ("precision ration:" + str(precision_ratio/5))
    print ("recall ration:" + str(true_predictions_ratio/5))
    print ("unknown predictions ration:" + str(unknown_predictions_ratio/5))
    
    workSheetTemp.write(tmpRow, 0,  alpha)
    workSheetTemp.write(tmpRow, 1,  precision_ratio/len(tapeindexes))
    ...
    for key in doctorDict:
        ...
        workSheetCategories.write(tmpRow2, 3, categoriesDoctorPrecision[key]/5)
        workSheetCategories.write(tmpRow2, 4, categoriesDoctorRecall[key]/5)
    ...
    for key in patientDict:
        ...
        workSheetCategories.write(tmpRow2, 3, categoriesPatientPresicion[key]/5)
        workSheetCategories.write(tmpRow2, 4, categoriesPatientRecall[key]/5)
...
def generateConclutions(ind, predicts,tokenizer2,test_data):    
    '''row = 1
    workbook2 =xlsxwriter.Workbook('C:\hi\dvir55.xlsx')
    worksheet2 = workbook2.add_worksheet()
    ...
    for i in range(0,len(predicts)):
        ...
        worksheet2.write(row, 0, test_data[row-1][0])
        worksheet2.write(row, 1, test_data[row-1][1])
    ...
    wb.save(outputsPath +'VisualMatrixNew_Common' +str(ind) +'.xls')
    #################################################### Indices per category ##############################################################################
    ...
    tape1= train[:tapeindexes[1]]
    tape1Tags = [i[1] for i in tape1]
...
# labels
    #DG.add_weighted_edges_from([('Start',str(test_data[0][1]),'0')])
    nx.draw_networkx_labels(DG, pos, font_size=6, font_family='sans-serif')
    for n in DG:
        DG.node[n]['name'] = n
    d = json_graph.node_link_data(DG)
    json.dump(d, open('document.json','w'))

    httpserver.load_url('force.html')
    plt.axis('off')
    #plt.show()
    nx.write_pajek(DG, "english_graph" +str(ind) +".net")
    plt.savefig("graph"+str(ind)+".png")  
    plt.clf()    
...
def compareRealAndModel(predicts, afterFitTextTest, tokenizer2, test_data,tapeNum):
    row = 1
    path = outputsPath + 'englishComparison_' + str(tapeNum)+ '.xlsx'
...
def predictionsUsingGraph(graph, model_predicts, tokenizer_dic):   
    ...
    for i in range (1,len(model_predicts)):
        for j in range (0,len(tokenizer_dic)):
                    ...
                    p = p[1:]
                    for opt in p:
                        prevCat = [key for key, value in tokenizer_dic.items() if value == tmpInd][0]
                        place = dict[prevCat]
                        #tmplis = graph[place]
                        lis = graph[place-1]
                        ...
                        proSum+= prevProb*opt
                        tmpInd+=1
                    b[i][j] = model_predicts[i][j]*proSum
    #print (b)
    i=0
    newPredictions = []
...
def main():
    workbookTemp1 =xlsxwriter.Workbook(outputsPath+'alphaTable_Common.xlsx')
    workSheetTemp = workbookTemp1.add_worksheet()
    workSheetTemp.write(0, 0,  "alpha")
    workSheetTemp.write(0, 1,  "precision")
    workSheetTemp.write(0, 2,  "recall")
    workSheetTemp.write(0, 3,  "unknown_predictions_ratio")
    
    
    workbookTempCat =xlsxwriter.Workbook(outputsPath+'alphaTable_Common_Categories.xlsx')
    ...
    workbookTempCat.close()
    workbookTemp1.close()
    
    #calculateComposites()
    #curModel = nueralModel()

    #randomForestModel()
    #graph = generateConclutions(model_predicts)
    #b = predictionsUsingGraph(graph, model_predicts, tokenizer_dic)
    #print (model_predicts[1])
    #print (np.argmax(model_predicts[0]))
    #print (np.argmax(model_predicts[1]))
    #print (b[1])
    #print (tokenizer_dic.items())
    
    
        
    #calculateComposites(predictions)
