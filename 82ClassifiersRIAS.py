...
train_data = []
workbook =xlrd.open_workbook('C:\hi\data.xlsx')
...
def kFoldNueralNetwork(f,alpha):
    ...
    workbookTemp1 =xlsxwriter.Workbook('tmpFile2.xlsx')
    ...
    workSheetTemp.write(tmpRow, 1,  true_predictions_ratio/5)
    workSheetTemp.write(tmpRow, 2,  unknown_predictions_ratio/5)
    workSheetTemp.write(tmpRow, 3,  1-(true_predictions_ratio/5+unknown_predictions_ratio/5))
    tmpRow+=1
    f.flush()
...
def generateConclutions(ind, predicts,tokenizer2,test_data):    
    ...
    wb.save('C:\hi\VisualMatrixNew' +str(ind) +'.xls')
    #################################################### Indices per category ##############################################################################
    ...
    workbook3 =xlsxwriter.Workbook('C:\hi\graph' +str(ind) +'.xlsx')
...
def compareRealAndModel(predicts, afterFitTextTest, tokenizer2, test_data,tapeNum):
    row = 1
    path = 'C:\hi' +'\\' + 'englishComperation_' + str(tapeNum)+ '.xlsx'
...
def main():
    workbookTemp1 =xlsxwriter.Workbook('tmpFile2.xlsx')
    f = open('file.txt','w')
    workSheetTemp2 = workbookTemp1.add_worksheet()
    workSheetTemp2.write(0, 0,  "alpha")
    workSheetTemp2.write(0, 1,  "true_predictions_ratio")
    workSheetTemp2.write(0, 2,  "unknown_predictions_ratio")
    workSheetTemp2.write(0, 3,  "false_predictions_ratio")
    workbookTemp1.close()
    f.write("alpha           true_predictions_ratio         unknown_predictions_ratio                 false_predictions_ratio")
    f.write("\n")
    f.flush()
    print ("jjjjjjjjjjjjjjjjj")
    for alpha in np.arange(0.2,0.4,0.03):    
