...
def parseWordtoXExcel(path):
    ...
    workbook =xlsxwriter.Workbook(outputsPath+'hebrew_data.xlsx')
...
def kFoldNueralNetwork(f,alpha,workSheetTemp):
    ...
    #f.flush()
    #row = 1
    workbook4 =xlsxwriter.Workbook(outputsPath + 'composites_common_hebrew.xlsx')
    ...
    '''
        ...
        for i in range (0,len(afterFitTextTest)):
            ...
            worksheet5.write(i+1, 1, test_data[i][1])
            worksheet5.write(i+1, 2, pred)
            worksheet5.write(i+1, 3,  actorsTest[i])
...
def compareRealAndModel(predicts, afterFitTextTest, tokenizer2, test_data,tapeNum):
    row = 1
    path = outputsPath + 'hebrewComperation_' + str(tapeNum)+'_Common'+ '.xlsx'
...
def generateConclutions(ind, predicts,tokenizer2,test_data):    
    ...
    wb.save(outputsPath +'VisualMatrixNew_Common_Hebrew' +str(ind) +'.xls')
    #################################################### Indices per category ##############################################################################
...
def main():
   ...
   # generateConclutions(afterFitTextTest,test_data,tokenizer2,tmpModel)
    ...
    workbookTemp1 =xlsxwriter.Workbook(outputsPath+'alphaTable_Common_Hebrew.xlsx')
